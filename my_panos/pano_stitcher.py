...
def warp_image(image, homography):
    ...
    stitch = cv2.warpPerspective(src=image, M=homography,
        dsize=(width, height))
...
def create_mosaic(images, origins):
    ...
    dist_sorted = sorted(mapped, key=lambda x:
        math.sqrt(x[0][0] ** 2 + x[0][1] ** 2), reverse=True)
    ...
    for image in y_sorted:
        height = max(height, -1 * y_sorted[0][0][1]
            + image[1].shape[0] + image[0][1])
